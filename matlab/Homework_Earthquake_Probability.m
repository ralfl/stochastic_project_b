...
e=4*(exp(-.25*t)-exp(-.5*t));       % Modulating Funktion e(t)
...
for i=1:length(om)
Sbb(i)=S0*(4*zeta_g^2*om_g^2*om(i)^2+om_g^4)/((om_g^2-om(i)^2)^2+4*zeta_g^2*om_g^2*om(i)^2);   % PSD
end
...
if max(abs(x))<.1
    ...
    samples(ii)=0;
        ...
        ppeak=findpeaks(abs(x),'MINPEAKHEIGHT',.1);
        nr(ii)=length(ppeak);
        
        nr_sample=find((abs(x))>.1);
        samples(ii)=length(nr_sample);
...
Pe=sum(nr)/(ii*L*dt)    % Probability - Peaks above threshold per Earthquake(in seconds)

% Pe_peaks=sum(nr)/(ii*L)
Pe_sample=sum(samples)/(ii*L)   %Probability - Samples above threshold for each Earthquake(in samples)
